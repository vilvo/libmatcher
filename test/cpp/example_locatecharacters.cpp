...
std::string SCREENSHOT = "../img/virtualkeyb_lowercase.png";
...
int main() {
    ...
    mquery.icon = "qwertyuiopasdfghjklzxcvbnm";
    mquery.resultimage = "/tmp/test_locate_characters.jpg";
        // Currently only OCR method is supported
    mquery.method = matcher::LocateMethod::OCR;

    // Match icon image with screenshot
    MatchResult *mresult = new MatchResult();
    cv::Ptr<cv::Mat> resultimg = matcher->locateCharacters(image, mquery,
                                                           mresult);
